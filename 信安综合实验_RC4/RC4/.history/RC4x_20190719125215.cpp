...
char *readCiphertext(const char *filepath) {}

char *writeFile(vector<int> msg, const char *filepath) {
  ofstream outstream;
  outstream.open(filepath);
  for (auto i : msg) outstream << hex << i << " ";
  outstream.close();
}
